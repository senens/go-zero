...
func (s *Redis) PipelinedGet(keys []string) (result []red.Cmder, err error) {
	return s.PipelinedGetCtx(context.Background(), keys)
}

// PipelinedGetCtx lets fn to execute pipeline get commands
func (s *Redis) PipelinedGetCtx(ctx context.Context, keys []string) (result []red.Cmder, err error) {
	conn, err := getRedis(s)
	if err != nil {
		return
	}
